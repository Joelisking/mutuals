...
export default function CartPage() {
  ...
  const [appliedDiscount, setAppliedDiscount] = useState<null | { code: string; discountAmount: number; type: string; value: number; isSitewide?: boolean }>(null);
  ...
  useEffect(() => {
    ...
    setAppliedDiscount((current) => {
      ...
      const discountAmount = sitewideDiscount.type === 'PERCENTAGE'
        ? Math.round((totalPrice * sitewideDiscount.value) / 100 * 100) / 100
        : Math.min(sitewideDiscount.value, totalPrice);
      return { code: sitewideDiscount.code, discountAmount, type: sitewideDiscount.type, value: sitewideDiscount.value, isSitewide: true };
    });
  ...
  const finalTotal = totalPrice - discountAmountApplied + (totalPrice >= 100 ? 0 : 10);
        ...
        <div className="min-h-screen bg-white pt-32 md:pt-36 px-4 md:px-8 lg:px-16">
          <div className="max-w-screen-2xl mx-auto py-16 md:py-24">
            ...
            <div className="bg-[#f8f8f8] border border-[rgba(0,0,0,0.08)] rounded p-12 text-center">
              <svg className="w-16 h-16 mx-auto mb-6 text-[rgba(0,0,0,0.2)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
              </svg>
              <h2 className="text-2xl text-[#050507] font-medium mb-3">Your cart is empty</h2>
              <p className="text-base text-[rgba(0,0,0,0.5)] mb-8 font-light">Discover our latest drops and exclusive pieces</p>
      ...
      <div className="min-h-screen bg-white pt-32 md:pt-36 px-4 md:px-8 lg:px-16">
        <div className="max-w-screen-2xl mx-auto py-16 md:py-24">
          <div className="flex items-center justify-between mb-12">
            ...
            <button onClick={clearCart} className="text-sm text-[rgba(0,0,0,0.4)] hover:text-red-500 transition-colors uppercase tracking-[0.12em] font-medium">
              Clear Cart
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-4">
              {items.map((item) => (
                <div key={`${item.product.id}-${item.size}`} className="bg-[#f8f8f8] border border-[rgba(0,0,0,0.08)] rounded-[4px] p-6">
                  <div className="flex gap-6">
                    <Link href={`/shop/${item.product.slug}`} className="shrink-0 w-32 h-32 rounded-[4px] overflow-hidden border border-[rgba(0,0,0,0.08)] hover:border-[rgba(0,0,0,0.2)] transition-all">
                      <img src={item.product.images[0]} alt={item.product.name} className="w-full h-full object-cover" />
                    </Link>

                    <div className="flex-1 flex flex-col justify-between">
                      <div>
                        <Link href={`/shop/${item.product.slug}`} className="text-[18px] md:text-[20px] text-[#050507] font-medium hover:text-[#1ecbe1] transition-colors mb-2 block">
                          {item.product.name}
                        </Link>
                      ...
                      <div className="flex items-center justify-between mt-4">
                        <div className="flex items-center border border-[rgba(0,0,0,0.12)] rounded-sm overflow-hidden">
                          <button onClick={() => updateQuantity(item.product.id, item.size, item.quantity - 1)} className="px-4 py-2 text-[#050507] hover:bg-[rgba(0,0,0,0.04)] transition-colors">-</button>
                          <span className="px-6 py-2 text-[#050507] border-x border-[rgba(0,0,0,0.12)]">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.product.id, item.size, item.quantity + 1)} className="px-4 py-2 text-[#050507] hover:bg-[rgba(0,0,0,0.04)] transition-colors">+</button>
                        </div>
                        <button onClick={() => removeFromCart(item.product.id, item.size)} className="text-[13px] text-[rgba(0,0,0,0.4)] hover:text-red-500 transition-colors uppercase tracking-[0.12em] font-medium">
                          Remove
                        </button>
                      </div>
                    </div>
              ...
              <div className="bg-[#f8f8f8] border border-[rgba(0,0,0,0.08)] rounded-[4px] p-6 sticky top-24">
                ...
                <div className="mb-4">
                  {appliedDiscount?.isSitewide ? (
                    <p className="text-[13px] text-[#1ecbe1] font-medium">
                      ✓ {appliedDiscount.value}{appliedDiscount.type === 'PERCENTAGE' ? '%' : '$'} off applied automatically
                    </p>
                  ) : (
                    <>
                ...
                <div className="space-y-4 mb-6">
                  <div className="flex justify-between text-[15px]">
                    <span className="text-[rgba(0,0,0,0.5)] font-light">Items ({totalItems})</span>
                    <span className="text-[#050507] font-medium">${totalPrice.toFixed(2)}</span>
                  </div>
                  {appliedDiscount && (
                    <div className="flex justify-between text-[15px]">
                      <span className="text-[#1ecbe1] font-light">Discount ({appliedDiscount.code})</span>
                      <span className="text-[#1ecbe1] font-medium">-${appliedDiscount.discountAmount.toFixed(2)}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-[15px]">
                    <span className="text-[rgba(0,0,0,0.5)] font-light">Shipping</span>
                    <span className="text-[#050507] font-medium">{totalPrice >= 100 ? "FREE" : "$10.00"}</span>
                  </div>
                  {totalPrice < 100 && (
                ...
                <div className="border-t border-[rgba(0,0,0,0.08)] pt-4 mb-6">
                  <div className="flex justify-between">
                    <span className="text-[18px] text-[#050507] font-medium uppercase tracking-[0.08em]">Total</span>
                    <span className="text-[24px] text-[#050507] font-medium">${finalTotal.toFixed(2)}</span>
                  </div>
                </div>
                ...
                <Link href="/shop" className="block w-full bg-white border border-[rgba(0,0,0,0.12)] hover:bg-[rgba(0,0,0,0.04)] transition-colors rounded-sm px-8 py-4 text-[13px] text-[#050507] uppercase tracking-[0.12em] font-medium text-center">
                  Continue Shopping
                </Link>

                <div className="mt-6 pt-6 border-t border-[rgba(0,0,0,0.08)] space-y-3 text-[12px] text-[rgba(0,0,0,0.4)] font-light">
                  <div className="flex items-start gap-2">
                    <svg className="w-4 h-4 shrink-0 mt-0.5" fill="none" viewBox="0 0 16 16">
                      <path d="M8 1L10.5 6L16 6.75L12 10.5L13 16L8 13.25L3 16L4 10.5L0 6.75L5.5 6L8 1Z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
                    </svg>
                  ...
                  <div className="flex items-start gap-2">
                    <svg className="w-4 h-4 shrink-0 mt-0.5" fill="none" viewBox="0 0 16 16">
                      ...
                      <path d="M8 2L8 8L12 8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
